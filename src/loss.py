...
def CAKL_loss(teacher_logits: torch.Tensor, student_logits: torch.Tensor, temperature: float=1.0) -> torch.Tensor:
    teacher_probs = f.softmax(teacher_logits / temperature, dim=1)
    student_log_probs = f.log_softmax(student_logits / temperature, dim=1)
    teacher_log_wages = torch.max(teacher_probs, dim=-1).values
    waged_dkl = (teacher_log_wages * f.kl_div(student_log_probs, teacher_probs,
...
if __name__ == "__main__":
    ...
    print(get_loss_function("CAKL")(teacher_logits, student_logits))
