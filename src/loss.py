...
# CEKL - Cross-Entropy plus Kullback-Leibler Divergence
# CrossEntropyWithoutKD - Cross-Entropy without Knowledge Distillation
LossFunctionType = Literal[
    "CrossEntropy", "CrossEntropyWithoutKD", "KL", "CAKL", "CEKL", "Wasserstein"
]
...
def wasserstein_loss(
    ...
    return (
        torch.abs(
            teacher_cdf.reshape((-1, teacher_cdf.size(-1)))
            - student_cdf.reshape((-1, student_cdf.size(-1)))
        )
        .sum(dim=-1)
        .mean(dim=0)
    )
...
def cross_entropy_plus_KL(
    ...
    teacher_probs = f.softmax(teacher_logits, dim=-1)
    return f.cross_entropy(student_logits, teacher_probs) + lbda * KL_loss(
